...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS excess_of_goal FROM projects
JOIN pledges ON projects.id = pledges.project_id
GROUP BY projects.title HAVING excess_of_goal >= 0;"
end
...
"SELECT projects.catgory, pledges.amount FROM projects
JOIN pledges ON projects.id = pledges.project_id
